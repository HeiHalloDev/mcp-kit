...
- **Each app says what it does not hold.** The new `neighbours` config names the sibling connections of the same product family, what each owns, the tools to look for and how to get in. The kit puts it in every server's instructions ("What is not here") and matches a gap report against it in the preview, so the answer arrives while the person is still asking — and confirming still files the gap, because a wrong guess must never swallow a report. Matching is whole words, both ends, and nothing is stemmed: list the plural if you want the plural. **The kit ships this empty and no default will ever fill it** — the list is read by one organisation's staff, and one client's app must never mention another's.
- **The shorter road, said at the moment it would help.** `hints.instead_of` maps a tool to the one that answers in a single call. Past `after` calls to it in one stretch of work (default 5, repeated every 15), the kit appends a sentence to that tool's own reply: *"That is 6 calls to legacy_get_node in this piece of work. `legacy_read_branch` reads a whole branch in one call."* The call is answered as normal — a nudge, never a refusal. Counted per stretch of work when frames are on, per token otherwise, and never across people.
- **Triage stops needing a database.** The gap list now carries each gap's id, the date it came in, the date each person reported it, and when and by whom a settled one was decided. `report_gap` returns the id it filed under.
- **A gap can be filed and settled in one call**, by privileged staff: pass `status` and `resolution` with the report. That is how something already answered elsewhere gets filed and answered together, instead of file, look up the id, move it.
- **Duplicates merge.** `report_gap` with `gap` (the duplicate) and `merge_into` (the gap it repeats) moves the reporters across, adds up the weight, keeps blocking if either was, and closes the duplicate pointing at the survivor. Matching is on the title, so people who phrased one thing differently each got a row; merging is the only way to make the count real.
- **A resolution can be reworded.** Re-stating the same status with a new resolution updates what the reporters read, instead of refusing with "already planned".
- **`php artisan mcp:inventory` re-pins the tool snapshot** (`--check` in CI). Since v1.11.6 `mcp:install` never overwrites a file that exists, which left the Guards failure recommending a command that could no longer do the job and editing JSON by hand as the only way out. The guard message now names this one.
- **Talking about a credential is not carrying one.** `SecretDetector` matched the words `token`, `password` and `secret` on their own, so a report about the bearer-token upload path — containing no token — was refused, and so was every gap about resetting a password. It now looks for a credential's shape: a name with a delimiter and a value, a value that looks like one, or a pasted Bearer, Sanctum, OpenAI, GitHub or Slack token, or a private key. A field named for a credential still counts when it holds an unbroken run of characters.
