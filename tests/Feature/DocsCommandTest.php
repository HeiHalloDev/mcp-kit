...
test('the reference lists every tool with server, domain, ability and type', function () {
    ...
    expect(array_column($tools, 'name'))->toBe(['admin_only', 'attach_file', 'list_things', 'log_event', 'update_thing', 'monthly_numbers'])
        ->and($tools[2]['server'])->toBe('acme')
        ->and($tools[2]['domain'])->toBe('General')
        ->and($tools[2]['ability'])->toBe('acme:things:read')
        ->and($tools[2]['writes'])->toBeFalse()
        ->and($tools[4]['annotations'])->toBe(['IsIdempotent'])
        ->and($reference->abilitiesFor(UpdateThingTool::class))->toBe(['acme:things:write'])
        ->and($reference->inventory())->toBe(['acme' => ['admin_only', 'attach_file', 'list_things', 'log_event', 'update_thing'], 'reports' => ['monthly_numbers']]);
});
...
test('mcp:docs writes between the markers, keeps prose and --check fails when stale', function () {
    ...
    $this->artisan('mcp:docs')->expectsOutputToContain('Regenerated 6 tools')->assertSuccessful();

    $document = (string) file_get_contents($path);

    expect($document)->toStartWith("---\nnav_title: Tools")
        ->toContain('Prose stays.')
        ->toContain('6 tools across 2 servers. 2 read, 4 write.')
